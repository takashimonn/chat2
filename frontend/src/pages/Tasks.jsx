...
const TeacherView = ({ initialTasks = [] }) => {
    ...
    window.handleViewDetails = async (submissionId) => {
      ...
      await Swal.fire({
                ...
                <div class="detail-section">
                  ...
                  <div class="detail-grid">
                    <div class="detail-item">
                      ...
                      <span class="value ${
                        submission.grade ? "status-graded" : "status-pending"
                      }">
        ...
        didOpen: () => {
          ...
          document
            .getElementById("updateGradeButton")
            .addEventListener("click", handleUpdateGrade);
        },
      });
    };
    ...
    window.handleGradeSubmission = async (submissionId) => {
      ...
      if (formValues) {
        ...
        handleViewSubmissions(
          task,
          await (
            await axiosInstance.get(`/api/submissions/task/${task._id}`)
          ).data
        );
      }
    };
  ...
  const fetchTasks = async () => {
    try {
      ...
      const uniqueTasks = Array.from(
        new Map(data.map((task) => [task._id, task])).values()
      );
  ...
  const fetchSubjects = async () => {
    try {
      ...
      console.error(
        "Error completo en fetchSubjects:",
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      const uniqueInitialTasks = Array.from(
        new Map(initialTasks.map((task) => [task._id, task])).values()
      );
      setTasks(uniqueInitialTasks);
      // Solo cargar las entregas si hay tareas iniciales
      if (uniqueInitialTasks.length > 0) {
        await Promise.all(
          uniqueInitialTasks.map((task) => fetchSubmissions(task._id))
        );
      }
      setIsLoading(false);
    };
    ...
    Swal.fire({
      ...
      html: `
        <div class="submission-details">
          <div class="detail-row"><strong>Tarea:</strong> <span>${
            task.title
          }</span></div>
          <div class="detail-row"><strong>Estudiante:</strong> <span>${
            submission.student?.username || "No disponible"
          }</span></div>
          <div class="detail-row"><strong>Estado:</strong> <span>${
            submission.status || "Pendiente"
          }</span></div>
          <div class="detail-row"><strong>Fecha de entrega:</strong> <span>${new Date(
            submission.createdAt
          ).toLocaleString()}</span></div>
          <div class="detail-row"><strong>Calificación:</strong> <span>${
            submission.grade ? `${submission.grade}/100` : "Sin calificar"
          }</span></div>
          <div class="detail-row"><strong>Retroalimentación:</strong> <span>${
            submission.feedback || "Sin retroalimentación"
          }</span></div>
          ${
            fileUrl
  ...
  const handleGradeTask = (task, student) => {
    ...
    const submission = submissions[task._id]?.find(
        ...
        <div class="grade-form">
          ...
          ${
            submission.grade
              ...
              <p><strong>Retroalimentación actual:</strong> ${
                submission.feedback || "Sin retroalimentación"
              }</p>
            </div>`
              : ""
...
const StudentView = ({ tasks }) => {
  ...
  const checkIfSubmitted = async (taskId) => {
    try {
      const response = await fetch(
        `http://localhost:4000/api/submissions/student/task/${taskId}`,
        {
          headers: {
  ...
  const handleSubmitTask = async (taskId) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:4000/api/submissions/task/${taskId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
